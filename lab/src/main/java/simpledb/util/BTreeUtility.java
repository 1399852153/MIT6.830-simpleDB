...
import simpledb.model.dbfile.BTreeFileEncoder;
...
import simpledb.model.page.BTreePage;
...
public class BTreeUtility {
		...
		public void run() {
			try {
				...
				while(c < count) {
					...
					IndexPredicate ipred = new IndexPredicate(Predicate.Op.EQUALS, t.getField(bf.keyField()));
	...
	static class BTreeReader extends Thread {
		...
		public void run() {
			try {
				while(true) {
					IndexPredicate ipred = new IndexPredicate(Predicate.Op.EQUALS, f);
					DbFileIterator it = bf.indexIterator(tid, ipred);
					it.open();
					int c = 0;
					while(it.hasNext()) {
	...
	public static class BTreeDeleter extends Thread {
		...
		public void run() {
			try {
				...
				IndexPredicate ipred = new IndexPredicate(Predicate.Op.EQUALS, key);
				DbFileIterator it = bf.indexIterator(tid, ipred);
				it.open();
				while(it.hasNext()) {
					Tuple t = it.next();
