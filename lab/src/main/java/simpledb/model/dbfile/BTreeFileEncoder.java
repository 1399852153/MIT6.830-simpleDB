...
import simpledb.model.page.BTreeInternalPage;
import simpledb.model.page.BTreeLeafPage;
import simpledb.model.page.BTreeRootPtrPage;
...
public class BTreeFileEncoder {
	...
	public static class TupleComparator implements Comparator<Tuple> {
		...
		public int compare(Tuple t1, Tuple t2) {
			int cmp = 0;
			if(t1.getField(keyField).compare(Predicate.Op.LESS_THAN, t2.getField(keyField))) {
				cmp = -1;
			}
			else if(t1.getField(keyField).compare(Predicate.Op.GREATER_THAN, t2.getField(keyField))) {
				cmp = 1;
			}
			return cmp;
		}
	...
	public static BTreeFile convert(File inFile, File hFile, File bFile, int npagebytes,
		...
		for(Tuple tup : tuples) {
			if(page1.size() < nrecords) {
				...
				// 最简单的情况下不会走到这里（tuples小于两页，page1+page2正好构成root节点的左右孩子页）
		...
		if(size > 0) {
			...
			if(size == nentries * 2 + 1) {
				...
				updateEntries(entries, bf, pushUpEntry, level + 1, nentries, npagebytes, 
						keyType, tableid, keyField);
				ArrayList<BTreeEntry> remainingEntries = new ArrayList<>(entries.get(level).subList(nentries + 1, size));
	...
	public static byte[] convertToLeafPage(ArrayList<Tuple> tuples, int npagebytes,
			int numFields, Type[] typeAr, int keyField)
					throws IOException {
		int nrecbytes = 0;
		// 计算每一个tuple所占用的byte大小
		for (int i = 0; i < numFields ; i++) {
			nrecbytes += typeAr[i].getLen();
		}
		// pointerbytes: left sibling pointer, right sibling pointer, parent pointer
		// 指针所占用的byte大小（左/右兄弟、双亲）
		int pointerbytes = 3 * BTreeLeafPage.INDEX_SIZE;
		// 每一页可以容纳的最大记录数 = （每一页的bit大小 - 当前页的三个指针bit大小）/ （每个tuple的bit大小 + 1bit的头部位图空间）
		...
		if (nheaderbytes * 8 < nrecords) {
			nheaderbytes++;  //ceiling 不足则必须向上取整，冗余一点空间
		}
		// 头部所占空间的bit大小
		...
		// 1. 按照parent、left sibling、right sibling的顺序写入三个指针
	...
	public static class EntryComparator implements Comparator<BTreeEntry> {
		...
		public int compare(BTreeEntry e1, BTreeEntry e2) {
			...
			else if(e1.getKey().compare(Predicate.Op.GREATER_THAN, e2.getKey())) {
				cmp = 1;
			}
			return cmp;
		}
	...
	public static class ReverseEntryComparator implements Comparator<BTreeEntry> {
		...
		public int compare(BTreeEntry e1, BTreeEntry e2) {
			...
			else if(e1.getKey().compare(Predicate.Op.LESS_THAN, e2.getKey())) {
				cmp = 1;
			}
			return cmp;
		}
