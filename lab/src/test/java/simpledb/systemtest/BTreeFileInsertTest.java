...
import simpledb.model.dbfile.BTreeFile;
...
BTreeFileInsertTest extends SimpleDbTestBase {
    ...
    public void addDuplicateTuples() throws Exception {
		...
		IndexPredicate ipred = new IndexPredicate(Predicate.Op.EQUALS, new IntField(3));
		...
		ipred = new IndexPredicate(Predicate.Op.GREATER_THAN_OR_EQ, new IntField(2));
		...
		ipred = new IndexPredicate(Predicate.Op.LESS_THAN, new IntField(2));
		it = empty.indexIterator(tid, ipred);
		it.open();
		count = 0;
		while(it.hasNext()) {
	...
	public void testSplitRootPage() throws Exception {
		...
		for(int i = 0; i < 100; i++) {
			...
			IndexPredicate ipred = new IndexPredicate(Predicate.Op.EQUALS, t.getField(0));
	...
	public void testSplitInternalPage() throws Exception {
		...
		while(fit.hasNext()) {
			Tuple tup = fit.next();
			if(prev != null)
				assertTrue(tup.getField(0).compare(Predicate.Op.GREATER_THAN_OR_EQ, prev.getField(0)));
			prev = tup;
			count++;
		}
		fit.close();
