...
public class BTreeFileReadTest extends SimpleDbTestBase {
    ...
    public void indexIterator() throws Exception {
		...
		ipred = new IndexPredicate(Predicate.Op.LESS_THAN_OR_EQ, f);
		it = twoLeafPageFile.indexIterator(tid, ipred);
		it.open();
		count = 0;
		while(it.hasNext()) {
			Tuple t = it.next();
			assertTrue(t.getField(0).compare(Predicate.Op.LESS_THAN_OR_EQ, f));
			count++;
		}
		...
		// equal to
		ipred = new IndexPredicate(Predicate.Op.EQUALS, f);
		it = twoLeafPageFile.indexIterator(tid, ipred);
		it.open();
		count = 0;
		while(it.hasNext()) {
			Tuple t = it.next();
			assertTrue(t.getField(0).compare(Predicate.Op.EQUALS, f));
			count++;
		}
		...
		f = new IntField(1000);
		ipred = new IndexPredicate(Predicate.Op.GREATER_THAN, f);
		it = twoLeafPageFile.indexIterator(tid, ipred);
		it.open();
		assertFalse(it.hasNext());
		it.close();
